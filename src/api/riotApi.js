...
export async function getLeagueEntries(tier, division = null, page = 1) {
  /**await delay(100);**/
  if (division) {
    // 디비전이 있는 티어 (IRON ~ DIAMOND)
    const url = `https://${REGION}.api.riotgames.com/lol/league/v4/entries/RANKED_SOLO_5x5/${tier}/${division}?page=${page}&api_key=${API_KEY}`;
    /**await delay(100);**/
    ...
    // 디비전이 없는 티어 (MASTER, GRANDMASTER, CHALLENGER)
    const url = `https://${REGION}.api.riotgames.com/lol/league/v4/${tier.toLowerCase()}leagues/by-queue/RANKED_SOLO_5x5?api_key=${API_KEY}`;
    const response = await fetch(url);
    if (!response.ok) throw new Error(`${tier} 리그 호출 실패: ${response.status}`);
    const data = await response.json();
    return data.entries || []; // 유저 리스트 반환
  }
}
...
export async function getMatchIdsByPuuid(puuid, count = 5) {
  const url = `https://${REGION_ASIA}.api.riotgames.com/lol/match/v5/matches/by-puuid/${puuid}/ids?start=0&count=${count}&api_key=${API_KEY}`;
  const response = await fetch(url);
  return await response.json();
}
...
export async function getMatchByMatchId(matchId) {
  const url = `https://${REGION_ASIA}.api.riotgames.com/lol/match/v5/matches/${matchId}?api_key=${API_KEY}`;
  const response = await fetch(url);
  return await response.json();
}
...
export async function getMatchTimelineByMatchId(matchId) {
  const url = `https://${REGION_ASIA}.api.riotgames.com/lol/match/v5/matches/${matchId}/timeline?api_key=${API_KEY}`;
  const response = await fetch(url);
  if (!response.ok) {
