...
function GoldGraph({ data }) {
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-[#121216]/95 backdrop-blur-xl border border-white/20 p-4 rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.6)] z-50 min-w-[220px] max-w-xs">
          {/* 상단: 시간 및 골드 차이 */}
          <div className="flex justify-between items-center mb-3 pb-2 border-b border-white/10">
            <span className="text-xs text-slate-400 font-bold uppercase tracking-wider">{time}분</span>
            <span className={`text-xs font-black ${isBlueLead ? 'text-blue-400' : 'text-red-400'}`}>
          ...
          <div className="space-y-1 mb-4">
            <div className="flex justify-between text-[11px]">
              ...
              <span className="text-slate-200 font-mono tracking-tight">{blueGold.toLocaleString()} G</span>
            </div>
            <div className="flex justify-between text-[11px]">
              <span className="text-red-400 font-bold">레드팀</span>
              <span className="text-slate-200 font-mono tracking-tight">{redGold.toLocaleString()} G</span>
            </div>
          </div>
              ...
              {matchEvents.map((event, idx) => (
                <div key={idx} className="flex items-center gap-2 animate-in fade-in slide-in-from-left-2 duration-300">
                  ...
                  <span className={`text-[10px] font-bold truncate ${
                    event.type === 'penta' ? 'text-yellow-400 animate-pulse' :
                    event.type === 'ace' ? 'text-orange-400' :
                    event.type === 'gold' ? 'text-yellow-200' :
                    event.type === 'obj' ? 'text-purple-300' :
                    'text-slate-200'
                  }`}>
                    {event.text}
                  </span>
  ...
  return (
    <div className="w-full bg-[#0a0a0c]/80 border border-white/5 rounded-2xl p-5 backdrop-blur-md">
      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2 opacity-80">
          ...
          <span className="text-[14px] font-black text-yellow-500 uppercase tracking-[0.2em]">
            시간대별 골드 격차 및 주요 사건
          </span>
        </div>
        ...
        <div className="flex gap-3 text-[10px] font-bold">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-blue-400">BLUE LEAD</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-red-500"></span>
            <span className="text-red-400">RED LEAD</span>
          </div>
        </div>
      </div>
          ...
          >
            <defs>
              <linearGradient id="splitColor" x1="0" y1="0" x2="0" y2="1">
                <stop offset={off} stopColor="#3b82f6" stopOpacity={0.5} />
      ...
      <div className="mt-4 pt-4 border-t border-white/10 grid grid-cols-2 gap-4">
        <div>
          <p className="text-[10px] text-slate-500 mb-1 uppercase tracking-wider font-bold">Max Lead</p>
          <p className="text-lg font-black text-white">{maxLead.toLocaleString()} G</p>
        </div>
        <div>
          <p className="text-[10px] text-slate-500 mb-1 uppercase tracking-wider font-bold">Final Difference</p>
          <div className="flex items-center gap-2">
            <span className={`text-lg font-black ${isBlueWin ? 'text-blue-400' : 'text-red-400'}`}>
              +{Math.abs(finalDiff).toLocaleString()} G
            </span>
            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${isBlueWin ? 'bg-blue-500/20 text-blue-300' : 'bg-red-500/20 text-red-300'}`}>
